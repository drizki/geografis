...
test('geometry.getNearest should return correct nearest village name', () => {
    const nearest = geografis.getNearest(myLatitude, myLongitude);
    expect(nearest.village).toBe('Cigadung'); 
})

test('geometry.getNearest should return correct nearest village code', () => {
    const nearest = geografis.getNearest(myLatitude, myLongitude);
    expect(nearest.code).toBe('32.73.18.1004'); 
})

test('geometry.getNearest should throw error if latitude or longitude is empty', () => {
    expect(() => geografis.getNearest()).toThrow('Parameter latitude and longitude is required');
})

test('geometry.getNearest should throw error if latitude or longitude is not number', () => {
